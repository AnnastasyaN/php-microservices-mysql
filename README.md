...
| Laragon | Menjalankan PHP dan MySQL secara lokal |
...
```bash
cd user-service
D:\laragon\bin\php\php-8.4.4-Win32-vs17-x64\php.exe -S localhost:8001
```

## Uji User Service

**Akses Langsung ke User Service (port 8001):**
...
**Atau Melalui API Gateway (port 8000):**

```http
GET http://localhost:8000/users
GET http://localhost:8000/users/detail?id=1
...
```bash
cd product-service
D:\laragon\bin\php\php-8.4.4-Win32-vs17-x64\php.exe -S localhost:8002
```

## Uji Product Service

**Akses Langsung ke Product Service (port 8002):**
...
GET http://localhost:8000/products/detail?id=1
...
```bash
cd order-service
D:\laragon\bin\php\php-8.4.4-Win32-vs17-x64\php.exe -S localhost:8003
```

## Uji Order Service

**Akses Langsung ke Order Service (port 8003):**
...
```bash
cd api-gateway
D:\laragon\bin\php\php-8.4.4-Win32-vs17-x64\php.exe -S localhost:8000
```
...
## Cara 1: Testing Service Secara Individual (Recommended untuk Development)

Jalankan hanya service yang ingin ditest, misal User Service:
...
http://localhost:8001/users/detail?id=1
```

**Keuntungan:**
- Cepat untuk development dan debugging
- Mudah fokus ke satu service
- Cocok untuk testing endpoint tertentu

## Cara 2: Testing via API Gateway (Recommended untuk Production Testing)

Jalankan semua 4 service di terminal berbeda seperti pada section "Menjalankan Semua Service", lalu akses semua via API Gateway:
...
**Keuntungan:**
- Test komunikasi antar service
- Test API Gateway routing
- Simulasi production environment
- Lihat status semua service sekaligus

# Daftar Endpoint

## Akses Langsung ke Service (Tanpa API Gateway)

| Service | Port | Endpoint | Method |
|---|---|---|---|
| User Service | 8001 | `http://localhost:8001/users` | GET |
| User Service | 8001 | `http://localhost:8001/users/detail?id=1` | GET |
| User Service | 8001 | `http://localhost:8001/users` | POST |
| Product Service | 8002 | `http://localhost:8002/products` | GET |
| Product Service | 8002 | `http://localhost:8002/products/detail?id=1` | GET |
| Product Service | 8002 | `http://localhost:8002/products` | POST |
| Product Service | 8002 | `http://localhost:8002/products/reduce-stock` | POST |
| Order Service | 8003 | `http://localhost:8003/orders` | GET |
| Order Service | 8003 | `http://localhost:8003/orders/detail?id=1` | GET |
| Order Service | 8003 | `http://localhost:8003/orders` | POST |
| Order Service | 8003 | `http://localhost:8003/orders/update-status` | POST |

## Daftar Endpoint Melalui API Gateway

**⚠️ Gunakan endpoint ini ketika semua service sudah berjalan (User Service, Product Service, Order Service, dan API Gateway)**
...
**⚠️ Pastikan semua 4 service sudah berjalan di terminal berbeda:**
- Terminal 1: API Gateway (port 8000)
- Terminal 2: User Service (port 8001)
- Terminal 3: Product Service (port 8002)
- Terminal 4: Order Service (port 8003)

Skenario berikut menggunakan **API Gateway (port 8000)**. Untuk testing langsung ke service tertentu, gunakan port yang sesuai (8001, 8002, atau 8003).
...
1. Pastikan MySQL pada Laragon sudah berjalan sebelum menjalankan service.
2. Pastikan setiap service berjalan pada port yang berbeda.
3. Jalankan service melalui path folder masing-masing.
4. **Akses Langsung ke Service**: Gunakan port service (8001, 8002, 8003) untuk testing individual service.
   - Contoh: `http://localhost:8001/users` (User Service langsung)
5. **Akses via API Gateway**: Gunakan port 8000 setelah semua service berjalan.
   - Contoh: `http://localhost:8000/users` (User Service via API Gateway)
   - API Gateway harus dijalankan terlebih dahulu sebelum testing via Gateway.
6. Jika terminal menampilkan pesan `command not found: php`, gunakan path PHP dari Laragon berikut.

```bash
D:\laragon\bin\php\php-8.4.4-Win32-vs17-x64\php.exe
```
